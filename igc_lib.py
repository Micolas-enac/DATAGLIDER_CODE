...
import library.viterbi as viterbi
import library.geo as geo
...
class Thermal:
    ...
    def distance(self, other):
        phi_a = (self.enter_fix.lat + self.exit_fix.lat) / 2
        phi_b = (other.enter_fix.lat + other.exit_fix.lat) / 2
        lbda_a = (self.enter_fix.lon + self.exit_fix.lon) / 2
        lbda_b = (other.enter_fix.lon + other.exit_fix.lon) / 2
        return ((phi_b-phi_a) ** 2 + (lbda_b-lbda_a) ** 2) ** 0.5
